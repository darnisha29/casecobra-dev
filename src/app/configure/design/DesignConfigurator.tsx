...
const DesignConfigurator = ({ configId, imageUrl, imageDimensions }: DesignConfiguratorProps) => {
    ...
    return (
        <div className=" relative mt-20 grid grid-cols-1  lg:grid-cols-3 mb-20 pb-20">
            <div className="relative lg: h-[45.5rem] md : [25.3rem] overflow-hidden col-span-2 w-full max-w-4xl flex items-center justify-center rounded-lg border-2 border-dashed border-gray-300 p-12 text-center  focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 ">
    ...
    <RadioGroup
      value={options.color.label}
      onChange={(val: string) => {
        const selectedColor = COLORS.find(color => color.label === val);
        setOptions(prev => ({
          ...prev,
    ...
    >
      <Label htmlFor="color-selection">Color: {options.color.label}</Label>
      <div id="color-selection" className='mt-3 flex items-center space-x-3'>
        {COLORS.map(color => (
          <Radio
            key={color.label}
            value={color.label}
